package com.paathena.chime_plugin
...
import com.paathena.chime_plugin.observers.*
import com.paathena.chime_plugin.views.PAAVideoRenderView
import com.paathena.chime_plugin.views.PAAVideoRenderViewFactory
...
class ChimePlugin : FlutterPlugin, MethodCallHandler {
  ...
  override fun onAttachedToEngine(binding: FlutterPluginBinding) {
    ...
    val safeMethodChannel = MethodChannel(messenger, "ChimePlugin")
    ...
    registry.registerViewFactory("PAAVideoRenderView", PAAVideoRenderViewFactory())
  }
  ...
  private fun handleClearViewIds(call: MethodCall, result: Result) {
    PAAVideoRenderViewFactory.clearViewIds()
    result.success(null)
  }
  ...
  private fun handleCreateMeetingSession(call: MethodCall, result: Result) {
    ...
    safeAudioVideoFacade.addActiveSpeakerObserver(DefaultActiveSpeakerPolicy(), PAAActiveSpeakerObserver(safeEventSink))
    safeAudioVideoFacade.addAudioVideoObserver(PAAAudioVideoObserver(safeEventSink))
    // addContentShareObserver: onContentShareStarted, onContentShareStopped
    safeAudioVideoFacade.addDeviceChangeObserver(PAADeviceChangeObserver(safeEventSink))
    // addEventAnalyticsObserver: onEventReceived
    safeAudioVideoFacade.addMetricsObserver(PAAMetricsObserver(safeEventSink))
    // addRealtimeDataMessageObserver: onDataMessageReceived
    safeAudioVideoFacade.addRealtimeObserver(PAARealtimeObserver(safeEventSink))
    safeAudioVideoFacade.addVideoTileObserver(PAAVideoTileObserver(safeEventSink))
  ...
  private fun handleBindVideoView(call: MethodCall, result: Result) {
    ...
    val view = PAAVideoRenderViewFactory.getViewById(viewId)
    if (view == null) {
